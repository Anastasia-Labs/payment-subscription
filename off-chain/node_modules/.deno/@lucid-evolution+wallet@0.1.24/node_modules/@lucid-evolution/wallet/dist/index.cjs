...
function walletFromSeed(seed, options = { addressType: "Base", accountIndex: 0, network: "Mainnet" }) {
  ...
  const address = options.addressType === "Base" ? CML.BaseAddress.new(
    networkId,
  ...
  const rewardAddress = options.addressType === "Base" ? CML.RewardAddress.new(
    networkId,
    CML.Credential.new_pub_key(stakeKeyHash)
...
var makeWalletFromSeed = (provider, network, seed, options) => {
  ...
  return {
    ...
    getUtxosCore: async () => {
      const utxos = config.overriddenUTxOs.length > 0 ? config.overriddenUTxOs : await provider.getUtxos((0, import_utils2.paymentCredentialOf)(address));
      const coreUtxos = [];
...
var makeWalletFromPrivateKey = (provider, network, privateKey) => {
  ...
  return {
    ...
    overrideUTxOs: (utxos) => void 0,
    address: async () => address,
    rewardAddress: async () => null,
    getUtxos: async () => {
      return await provider.getUtxos((0, import_utils2.paymentCredentialOf)(address));
    },
    getUtxosCore: async () => {
      const utxos = await provider.getUtxos((0, import_utils2.paymentCredentialOf)(address));
      const coreUtxos = [];
      utxos.forEach((utxo) => {
        coreUtxos.push((0, import_utils2.utxoToCore)(utxo));
      });
    ...
    getUtxos: async () => {
      const utxos = (await api.getUtxos() || []).map((utxo) => {
        const parsedUtxo = CML.TransactionUnspentOutput.from_cbor_bytes(
          (0, import_core_utils2.fromHex)(utxo)
        );
        return (0, import_utils2.coreToUtxo)(parsedUtxo);
      });
      return utxos;
    },
    getUtxosCore: async () => {
...
var makeWalletFromAddress = (provider, network, address, utxos) => {
  ...
  return {
    //TODO: implement utxos
    overrideUTxOs: (utxos2) => void 0,
    address: async () => address,
    rewardAddress: async () => rewardAddress,
    getUtxos: async () => utxos,
    getUtxosCore: async () => utxos.map(import_utils2.utxoToCore),
