...
var makeWalletFromSeed = (provider, network, seed, options) => {
  ...
  return {
    ...
    getUtxosCore: async () => {
      const utxos = config.overriddenUTxOs.length > 0 ? config.overriddenUTxOs : await provider.getUtxos(paymentCredentialOf(address));
      const coreUtxos = [];
...
var makeWalletFromAddress = (provider, network, address, utxos) => {
  ...
  return {
    ...
    getUtxosCore: async () => utxos.map(utxoToCore),
