...
var Maestro = class {
  ...
  async submitTx(tx) {
    ...
    const response = await fetch(queryUrl, {
      ...
      body: (0, import_core_utils2.fromHex)(tx)
    });
  ...
  maestroUtxoToUtxo(result) {
    return {
      ...
      scriptRef: result.reference_script ? result.reference_script.type == "native" ? void 0 : {
        ...
        script: (0, import_utils3.applyDoubleCborEncoding)(result.reference_script.bytes)
      } : void 0
    };
  }
...
var Koios = class {
  ...
  async getUtxoByUnit(unit) {
    let { policyId, assetName } = (0, import_utils4.fromUnit)(unit);
  ...
  awaitTx(txHash, checkInterval = 2e4) {
    ...
    const program = (0, import_effect.pipe)(
      import_effect.Effect.tryPromise({
        //TODO: use tx_status api
        try: () => fetch(`${this.baseUrl}/tx_info`, request),
        catch: (error) => new Error(String(error))
      }),
      import_effect.Effect.flatMap(
        (response) => !response.ok ? import_effect.Effect.fail(new Error(`${response.status} ${response.statusText}`)) : import_effect.Effect.succeed(response)
      ),
      import_effect.Effect.flatMap(
        (response) => import_effect.Effect.tryPromise({
          try: () => response.json(),
          catch: (error) => new Error(String(error))
        })
      ),
      ...
      import_effect.Effect.catchTag(
        "ParseError",
        (e) => import_effect.Effect.fail(import_schema2.ArrayFormatter.formatErrorSync(e))
      ),
      import_effect.Effect.repeat({
        schedule: import_effect.Schedule.exponential(checkInterval),
        until: (result) => result.length > 0
      }),
      import_effect.Effect.orDie,
      import_effect.Effect.as(true)
    );
...
var toUTxO = (koiosUTxO, address) => ({
  ...
  scriptRef: koiosUTxO.reference_script ? koiosUTxO.reference_script.type == "plutusV1" ? {
    type: "PlutusV1",
    script: (0, import_utils4.applyDoubleCborEncoding)(koiosUTxO.reference_script.bytes)
  } : {
    type: "PlutusV2",
    script: (0, import_utils4.applyDoubleCborEncoding)(koiosUTxO.reference_script.bytes)
  } : void 0
